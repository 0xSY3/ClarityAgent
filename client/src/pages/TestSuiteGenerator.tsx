...
export default function TestSuiteGenerator() {
  ...
  const generateMutation = useMutation<GeneratedTest[], Error>({
    mutationFn: async () => {
  ...
  const generateTests = async () => {
    ...
    try {
      setIsLoading(true);
      const generatedTests = await generateMutation.mutateAsync();
              ...
              <Card className="border-orange-500/20 bg-orange-900/10 backdrop-blur-sm">
                ...
                <CardContent className="space-y-4">
                  <Textarea
                    ...
                    className="font-mono min-h-[400px] bg-orange-500/10 border-orange-500/20 text-white placeholder:text-white/40"
                  />
                  <Button
                    onClick={generateTests}
                    disabled={isLoading || !contractCode}
                    className="w-full bg-orange-600/90 text-white hover:bg-orange-500 border border-orange-500/30 shadow-lg shadow-orange-500/20"
                  >
                    {isLoading ? (
                      <>
                  ...
                  <div className="space-y-4">
                    {tests.length === 0 ? (
                      ...
                      tests.map((test, index) => (
                        <Card key={index} className="border-orange-500/20 bg-orange-900/10 backdrop-blur-sm transition-all hover:border-orange-500/40 animate-in fade-in-50 slide-in-from-bottom-5 duration-500" style={{ animationDelay: `${index * 100}ms` }}>
                          ...
                          <CardContent className="space-y-4">
                            <div className="space-y-2">
                              ...
                              <div className="text-sm bg-orange-500/10 p-3 rounded-md border border-orange-500/20">
