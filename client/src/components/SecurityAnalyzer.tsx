  ...
  const analyzeContract = async () => {
    ...
    try {
      const response = await fetch('/api/ai/analyze', {
        ...
        body: JSON.stringify({ 
          code,
          contractType: 'clarity' // Specify contract type
        })
      });
      ...
      setAnalysisResults([{
        ...
        description: error instanceof Error ? 
          error.message : 
          'Failed to perform security analysis. Please try again.',
      }]);
    ...
    <div className="space-y-4">
      <Button
        ...
        className="w-full bg-orange-600/90 text-white hover:bg-orange-500 
          border border-orange-500/30 shadow-lg shadow-orange-700/20 
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            {analysisResults.map((issue, index) => (
              <div
                key={index}
                className={`p-4 rounded-lg border overflow-hidden break-words ${getSeverityColor(issue.severity)}`}
              >
                <div className="flex items-start gap-3">
                  {getSeverityIcon(issue.severity)}
                  <div className="flex-1 min-w-0"> {/* Add min-w-0 for proper text wrapping */}
                    ...
                    {issue.snippet && (
                      <div className="my-3 p-3 rounded bg-black/50 font-mono text-sm">
                    ...
                    {issue.impact && (
                      <div className="mt-2">
                        <span className="text-sm font-medium block">Impact:</span>
